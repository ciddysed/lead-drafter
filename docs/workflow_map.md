...
- Every new lead requires manual attention before first contact goes out.
  This doesn't scale past a handful of leads per day without either
  quality dropping (generic templates) or response time slowing (a
  growing backlog).
...
- Not simulating the actual sending of email/SMS (no email/SMS provider
  integration). The system produces drafts and a review decision;
