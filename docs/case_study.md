...
This version is rebuilt from scratch with original code and 100%
synthetic data.
...
- Every new lead requires manual attention before first contact goes
  out, and this doesn't scale past a handful of leads per day without
...
(lead record leads to personalized draft leads to confidence/flag-based
human review before anything sends) applies to any business doing
individualized outbound lead outreach: real estate, recruiting,
...
baseline (specifically on the Grounding and Safety dimensions, detailed
in Results below), plus correct review-queue routing, where every test
...
- **Confidence threshold for review, not a fixed rule list:** lets the
  ...
  than requiring me to enumerate every possible flag-worthy condition in
  advance. The trade-off: the model's confidence self-reports are
  unverified (a documented limitation).
- **No LLM-graded evaluation:** rubric scoring in this case study was
  ...
  its own (or another LLM's) output. This avoided introducing a second,
  unvalidated LLM judgment layer into a 5-day scope.
- **No RAG:** considered and deliberately rejected. RAG solves the
  problem of there being more relevant knowledge than fits in one
  prompt, knowledge that also changes over time. This problem doesn't
  have that shape: the entire "knowledge" needed for any single draft is
  one row of lead data, which already fits trivially in the prompt, so
  there's no corpus to retrieve from. Building a retrieval pipeline over
  data with nowhere to retrieve *from* would be unjustified complexity,
  not more sophisticated engineering.
...
- **TC01**: invented specific claims not in the lead data at all: "we
  ...
  urgency isn't just a quality miss, it's a dishonesty and
  legal-exposure risk if it actually reached a customer. Re-verified
  after giving baseline the real company name/business description too
  (see below, Failures section): it still invented the same
...
- **TC04**: the limit case. A lead with no name, no context, no contact
  ...
  other thin-data case, it's "refuse to draft at all," since there's not
  even a name or contact method to route to a human. Baseline drafted a
  full, polished-sounding pitch anyway. System correctly raised
  `ValueError` instead.
- **TC06**: baseline didn't actually comply with the embedded injection
...
only allows 15 requests/minute, which the eval script's original
1-second pacing blew straight through. This is a real, practical
limitation for anyone trying to build and evaluate an LLM system on a
free API tier: budget for a paid tier before real production use.
...
4. `run_eval.py` read `test_cases.json` with Windows' default cp1252
   encoding, which can't decode TC07's Vietnamese text, crashing before
...
7. Windows console defaults to cp1252 for `print()` too. The CLI itself
   would crash showing any lead with non-ASCII content, not just the
   eval script.
...
- **Gap-messaging rule**: added an explicit system-prompt rule
  ...
  contact method, where the draft now proactively asks for it) from
  gaps that must stay internal-only (contradictions, injection attempts,
  identity/ethics ambiguity, confirmed these still never get surfaced to
  the lead directly after the change).
...
- **Independent grounding-verification pass**: a second LLM call (same
  ...
  the draft would otherwise auto-approve. A low-confidence/flagged draft
  already goes to review regardless, so the value is catching
...
- **Business description config**: considered and rejected hardcoding
  ...
  general-purpose design (any individualized outbound lead outreach, not
  just this niche) and risked reintroducing grounding violations, since
  a lead with no surplus-related context would still get framed around
  surplus funds regardless of their actual data. Added
...
**Verified, not just assumed:** two leads with identical names ("Maria
Santos") but different `lead_id`s never got conflated by the sheet
lookup, checked directly rather than taken on faith.
...
- **Self-reported confidence is still not fully independently verified.**
  ...
  fix. It doesn't second-guess tone judgments, flag-worthiness calls, or
  the confidence *number* itself, and it's only one more LLM call
...
- **Duplicate `lead_id` rows in the `Leads` tab aren't detected.** Found
  ...
  `sheets_client.get_lead()` would also silently return only the first
  match if ever called elsewhere. Lower severity than the fixes above (a
  data-entry mistake, not a compliance or fabrication risk), documented
  rather than fixed, given limited remaining time.
...
2. Add contact-format validation as a pre-send gate (email/phone regex
   check), since it currently drafts anyway and only flags it.
