...
It's deliberately manual-only, not on an automatic schedule, since an
...
Setup, in your GitHub repo settings under Secrets and variables, then Actions:
...
3. Check the Actions tab's run logs to see what happened on each run.
   This is your production monitoring and logging for this workflow.

This is free in practice (GitHub Actions provides free minutes on a
public repo) and has no cold-start or sleep issue, unlike a free-tier
web service, which fits well since this workflow is trigger/batch-style,
