...
## 0:00 - 0:50 -- The problem and baseline
...
- "The baseline for comparison is simple ChatGPT-style use -- one
  generic prompt, no rules." (Show one baseline example on screen --
  TC01's baseline output with the invented "contingency basis, no
  upfront costs" claim highlighted.)
- "Even on the easiest case, it invents business terms nobody gave it.
  That's not a quality nitpick -- a real business sending fabricated
  fee claims is a trust and legal-exposure risk."

## 0:50 - 2:00 -- Live flow, real input to output
...
- Switch back to the Sheet's `Drafts` tab -- show the new row: draft
...
## 2:00 - 3:00 -- The non-developer user experience
...
## 3:00 - 4:00 -- Evaluation and failure handling
...
## 4:00 - 4:45 -- Results and the most important limitation
...
- State the single most important limitation, plainly: "The whole
  ...
  that's a partial mitigation, not a complete fix -- it's still one
  more LLM checking another LLM, not a deterministic guarantee."

## 4:45 - 5:00 -- Close

- "Next priority is adding retry/backoff to the live CLI -- I hit a
  real transient API failure during testing today, so that's not a
